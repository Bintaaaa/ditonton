...
class ShareCretificate {
  static Future<HttpClient> customeHttpClient({bool isTestMode = false}) async {
    ...
    try {
      ...
      if (isTestMode) {
        certFileBytes = utf8.encode(_certificateSSL);
      } else {
        try {
          certFileBytes =
  ...
  static Future<http.Client> createLEClient({bool isTestMode = false}) async {
    IOClient client = IOClient(
        await ShareCretificate.customeHttpClient(isTestMode: isTestMode));
    return client;
  }
}

const _certificateSSL = """-----BEGIN CERTIFICATE-----
